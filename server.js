...
/* ════════════════════════════════════════════════════════════════════
   Google Drive backup — service-account.json must sit in project root.
   Set DRIVE_BACKUP_FOLDER_ID in .env.
════════════════════════════════════════════════════════════════════ */
let _driveAuth   = null;
...
function _initDrive() {
  ...
  try {
    ...
    const saKey = require(path.join(__dirname, 'service-account.json'));
...
async function _handleBackupPdf(req, res) {
  ...
  req.on('end', async () => {
    ...
    if (!_initDrive()) {
      res.writeHead(500, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ error: 'Drive not initialised — check service-account.json' }));
    }
